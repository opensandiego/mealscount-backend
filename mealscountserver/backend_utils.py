...
# data instances
#
class dataFrameAndMetadataFromXL(mcSchoolDistInput):
    """
    ...
    def __init__(self, datafile):
        ...
        try:
            self.parseXL(self.__datafile)
        except Exception as e:
            raise e

    def to_frame(self):
        return prepare_data(self.d_df)
    ...
    def parseXL(self, xlfile):
        ...
        try:
            ...
            # rows at this point
            df_full = pd.concat(df_list, axis=0, ignore_index=True, sort=False)
            ...
            self.d_df = prepare_data(df_full)
...
def prepare_data(df):
    ...
    NUMERIC_COLS = set(['total_enrolled', 'frpm', 'foster', 'homeless', 'migrant', 'direct_cert'])
    all_cols = set(df.columns)
    remaining = all_cols.intersection(NUMERIC_COLS)

    df = df.copy()
    df[list(remaining)] = df[list(remaining)].apply(pd.to_numeric)

    # remove aggregated records
    df = df[df['school_name'] != 'total']

    # sum cols for homeless, migrant and foster students
    df['non_direct_cert'] = df['foster'] + df['homeless'] + df['migrant']

    # compute total eligible and isp

    df['total_eligible'] = df['foster'] + df['homeless'] + df['migrant'] + df['direct_cert']
    df['isp'] = (df.total_eligible / df['total_enrolled']) * 100
    df['isp'] = df['isp'].astype(np.double)

    KEEP_COLS = ['school_code', 'total_enrolled', 'direct_cert', 'non_direct_cert', 'total_eligible', 'isp', 'school_name', 'foster']

    # remove cols not needed for further analysis
#    drop_cols = set(df.columns.tolist()) - set(KEEP_COLS)
